...
class DatabaseStorage:
    ...
    def __init__(self):
        # Tables are created during migration or startup
        pass
...
async def send_message(request: ChatRequest, current_user_id: str = Security(get_current_user_id)):
    try:
        ...
        user_message = ChatMessage(id=str(uuid.uuid4()), content=request.message, sender="user", timestamp=datetime.now(), session_id=session_id)
        chat_sessions[session_id].append(user_message)

        ai_response_content = await generate_ai_response(request.message)
        ai_message = ChatMessage(id=str(uuid.uuid4()), content=ai_response_content, sender="ai", timestamp=datetime.now(), session_id=session_id)
